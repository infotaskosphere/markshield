...
export default function CalendarPage({ context }) {
  ...
  const eventsForDay = (day) =>
    events.filter(e => e.year === viewYear && e.month === viewMonth && e.day === day)
  ...
  const typeColor = { hearing: "var(--accent-light)", deadline: "var(--rose)", renewal: "var(--teal)", other: "var(--sky)" }
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 280px", gap: 16 }}>
      ...
      <div className="card" style={{ margin: 0 }}>
        <div className="card-head" style={{ alignItems: "center" }}>
          <div className="cal-month-nav" style={{ flex: 1, margin: 0 }}>
          ...
          <button className="topbar-btn btn-primary" style={{ fontSize: 12 }} onClick={() => { setSelDay(now.getDate()); setShowAdd(true) }}>
            + Add Event
          </button>
        </div>
            ...
            {Array.from({ length: daysInMonth }, (_, i) => {
              ...
              return (
                ...
                >
                  ...
                  {dayEvents.map(ev => (
                    <div key={ev.id} className={`cal-event-dot${ev.type === "deadline" ? " rose" : ev.type === "renewal" ? " teal" : ""}`} title={ev.title}>
                      {ev.title}
                    </div>
                  ))}
                </div>
        ...
        {upcoming.length === 0 ? (
          <div className="card" style={{ padding: 24, margin: 0, textAlign: "center" }}>
            ...
            <div style={{ fontSize: 13, color: "var(--text3)" }}>
              No upcoming events. Click a date to add hearings, deadlines, or renewals.
            </div>
          </div>
        ) : (
            ...
            {upcoming.map(ev => (
              <div key={ev.id} className="upcoming-item">
                ...
                <div className="upcoming-body">
                  ...
                  <div className="ub-sub" style={{ color: typeColor[ev.type] || "var(--text3)" }}>{ev.type}{ev.app ? ` · ${ev.app}` : ""}</div>
                </div>
                <button onClick={() => removeEvent(ev.id)} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--text3)", fontSize: 14 }}>✕</button>
              </div>
            ))}
          </div>
        )}
        ...
        <div className="overlay open" onClick={e => e.target.classList.contains("overlay") && setShowAdd(false)}>
          <div className="modal">
            ...
            <div className="modal-sub">
              {selDay && `${MONTHS[viewMonth]} ${selDay}, ${viewYear}`}
            </div>
            <div className="mf">
              <label>Event Title *</label>
              <input placeholder="e.g. TECHVEDA Hearing" value={form.title} onChange={e => setForm(p => ({...p, title: e.target.value}))} autoFocus />
            </div>
            <div className="mf">
              <label>Type</label>
